...
class Vocab(object):
  ...
  def IdToWord(self, word_id):
    print "len:",len(self._id_to_word)
    if word_id not in self._id_to_word:
      print ":",len(self._id_to_word),":word_id:",word_id
...
def ExampleGen(data_path, mode=None, num_epochs=None):
...
def GetWordIds(text, vocab, pad_len=None, pad_id=None):
  ...
  for w in text.split():
    ...
    if i >= 0:
      ...
      print "error:",w,"did not exist in dic"
...
ExampleGen("./result_luna.bin","epoch")
