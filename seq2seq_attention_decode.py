...
tf.app.flags.DEFINE_integer('max_decode_steps', 1,#1000
                            'Number of decoding steps.')
tf.app.flags.DEFINE_integer('decode_batches_per_ckpt', 4,
                            'Number of batches to decode before restoring next '
                            'checkpoint')

DECODE_LOOP_DELAY_SECS = 2
...
class DecodeIO(object):
  ...
  def Write(self, reference, decode, filename):
    ...
    #in terms of cccs output format
    self._decode_file.write('%s\tFz:0\t' % filename)
    self._decode_file.write('%s\n' % decode)
    self._cnt += 1
...
class BSDecoder(object):
  ...
  def _getckptFiles(self):
    f=codecs.open(FLAGS.log_root+"/checkpoint","r")
    filenames=[]
    for line in f:
      if line.find("all_model_checkpoint_paths")<>-1:
        item=line.split(" ")
        filename=item[1].replace('"','')
        filename =filename.replace("\n","")
        filenames.append(FLAGS.log_root+"/"+filename)
    return filenames
  ...
  def _Decode(self, saver, sess):
    ...
    filenames=self._getckptFiles()
    initWE=None
    if FLAGS.word2vec <> None:
      initWE=self._model._loadWord2Vec()
    for filename in filenames:
      saver.restore(sess, filename) #ckpt_path
      self._decode_io = DecodeIO(filename+"Dir")
      self._decode_io.ResetFiles()
      self._batch_reader._FillBucketInputQueueShuffle();
      while self._batch_reader._bucket_input_queue.qsize()<>0:
        (article_batch, _, _, article_lens, _, _, origin_articles,
          origin_abstracts,origin_filenames) = self._batch_reader.NextBatch()
        for i in xrange(self._hps.batch_size):
          bs = beam_search.BeamSearch(
            ...
            self._hps.dec_timesteps,initWE)
          ...
          self._DecodeBatch(
            origin_articles[i], origin_abstracts[i], origin_filenames[i], decode_output)

    return True

  def _DecodeBatch(self, article, abstract, filename, output_ids):
    ...
    tf.logging.info('filename: %s', filename)
    tf.logging.info('decoded:  %s', decoded_output)
    self._decode_io.Write(abstract, decoded_output.strip(),filename) # origin_abstract  decode_abstract
