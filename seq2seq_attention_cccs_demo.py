...
import batch_reader
import data
import seq2seq_attention_decode_demo
import seq2seq_attention_model_demo
...
tf.app.flags.DEFINE_integer('EARLY_STOP_PATIENCE',70, 'early stopping')
...
def _Train(model, data_batcher,test_batcher):
  ...
  with tf.device('/gpu:0'):
    ...
    for i in xrange(FLAGS.EPOCHS):
      ...
      running_avg_loss=validation(sess,model,test_batcher)
...
def validation(sess,model,test_batcher,vocab=None):
  """Runs model eval."""
  running_avg_loss = 0
  step = 0
  test_batcher._FillBucketInputQueueShuffle();
...
def main(unused_argv):
  ...
  hps = seq2seq_attention_model_demo.HParams(
      ...
      emb_dim=40,  # If 0, don't use embedding
      max_grad_norm=2,
  ...
  if hps.mode == 'train':
    ...
    model = seq2seq_attention_model_demo.Seq2SeqAttentionModel(
    ...
    print "read to create BSDecoder ....model loaded"
    decoder = seq2seq_attention_decode_demo.BSDecoder(model, batcher, hps, vocab)
