...
tf.app.flags.DEFINE_integer("svd_dim", None, "reduce dimension for svd factorization")
...
class Seq2SeqAttentionModel(object):
  ...
  def run_train_step(self, sess, article_batch, abstract_batch, targets,
                     article_lens, abstract_lens, loss_weights, initWE):
  ...
  def _add_placeholders(self):
    ...
    self._loss_weights = tf.placeholder(tf.float32,
                                        [hps.batch_size, hps.dec_timesteps],
                                        name='loss_weights')
    self._embedding = tf.placeholder(tf.float32,[self._vocab.NumIds(), hps.emb_dim],
                                        name='embedding')
  ...
  def _loadWord2Vec(self):
    ...
    if FLAGS.word2vec:
        ...
        initWE = np.random.uniform(-0.25,0.25,(vsize, emb_dim)).astype(np.float32)
        ...
        for line in f:
           ...
           if idx != None:
              initWE[idx] = np.fromstring(value, dtype='float32',sep=' ')
    ...
    if FLAGS.word2vec:
        ...
        with open(FLAGS.word2vec, "rb") as f:
            ...
            for line in xrange(vocab_size):
                ...
                idx = data.GetWordIds(word,self._vocab)
                if idx != None:
                    initWE[idx] = np.fromstring(f.read(binary_len), dtype='float32')  
                else:
                    f.read(binary_len)    
        f.close()
        print "initWe loaded:",initWE
  ...
  #This is for testing whether seq2seq model has loaded word embedding
  def _add_seq2seq_test(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      #  sess.run(tf.initialize_all_variables())
       ...
       # print(sess.run(emb_encoder_inputs))
        print sess.run(self._embedding,fedd_dict={self._embedding:initW})
        print "to test ... .. . . embedding  loaded:"
    ...
    with tf.variable_scope('seq2seq'):
      ...
      if FLAGS.word2vec == None:
        ...
        emb_encoder_inputs = [tf.nn.embedding_lookup(self._embedding, x)
                              for x in encoder_inputs]
        emb_decoder_inputs = [tf.nn.embedding_lookup(self._embedding, x)
                              for x in decoder_inputs]
      
      if FLAGS.svd_dim:
        #svd factorization
        svd_dim=FLAGS.svd_dim
        emb_encoder_matrix=tf.pack([tf.transpose(x) for x in emb_encoder_inputs])
        ...
        c=[tf.slice(tf.transpose(x),[0,0],[svd_dim,hps.emb_dim]) for x in tf.unpack(u)]
        c=tf.pack(c)
  ...
  def encode_top_state(self, sess, enc_inputs, enc_len, initWE):
  ...
  def decode_topk(self, sess, latest_tokens, enc_top_states, dec_init_states, initWE):
