...
ModelInput = namedtuple('ModelInput',
                        ...
                        'origin_article origin_abstract origin_filename')

BUCKET_CACHE_BATCH = 300
QUEUE_NUM_BATCH = 300
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, hps,
               article_key, abstract_key,filename_key,  max_article_sentences,
               max_abstract_sentences, bucketing=True, truncate_input=False,mode=None):
    """Batcher constructor.
    ...
    if self._mode <> "epoch":
        ...
        #inputs for random epoch
        print "start to load data to _input_queue"
        self.randomInputs=[]
  ...
  def NextBatch(self):
    ...
    origin_filename = ['None'] * self._hps.batch_size
    print "self._bucket_input_queue:",self._bucket_input_queue.qsize()
    buckets = self._bucket_input_queue.get()
    
    import hashlib
    for i in xrange(self._hps.batch_size):
      print "batch_size:",self._hps.batch_size
      print "=================",i
      (enc_inputs, dec_inputs, targets, enc_input_len, dec_output_len,
       article, abstract, filename) = buckets[i]
      
      origin_articles[i] = article
      origin_abstracts[i] = abstract
      origin_filename[i] = filename
    ...
    return (enc_batch, dec_batch, target_batch, enc_input_lens, dec_output_lens,
            loss_weights, origin_articles, origin_abstracts,origin_filename)
  ...
  def _FillInputQueue(self):
    ...
    input_gen = self._TextGenerator(data.ExampleGen(self._data_path,self._mode))
    while True:
      try:
        (article, abstract,filename) = input_gen.next()
      ...
      if (len(enc_inputs) < self._hps.min_input_len or
        ...
        #tf.logging.warning('Drop an example - too short.\nenc:%d\ndec:%d\nmin:%d\n',
         #                    len(enc_inputs), len(dec_inputs),self._hps.min_input_len)
        continue
      ...
      if not self._truncate_input:
        if (len(enc_inputs) > self._hps.enc_timesteps or
          ...
          #tf.logging.warning('cut an example - too long.\nenc:%d\ndec:%d\nenc:%d\ndec:%d\n',
                       #      len(enc_inputs), len(dec_inputs),self._hps.enc_timesteps,self._hps.dec_timesteps)
          
          if len(enc_inputs) > self._hps.enc_timesteps:
              enc_inputs=enc_inputs[0:self._hps.enc_timesteps]
          
          if len(dec_inputs) > self._hps.dec_timesteps:
              dec_inputs=dec_inputs[0:self._hps.dec_timesteps]
          #continue
      ...
      element = ModelInput(enc_inputs, dec_inputs, targets, enc_input_len,
                           ...
                           ' '.join(abstract_sentences),filename)
  ...
  def _FillBucketInputQueueShuffle(self):
      if len(self.randomInputs)==0:
        for _ in xrange(self._input_queue.qsize()):
          self.randomInputs.append(self._input_queue.get())
      
      shuffle(self.randomInputs)
      batches = []
      for i in xrange(0, len(self.randomInputs), self._hps.batch_size):
        batches.append(self.randomInputs[i:i+self._hps.batch_size])
      for b in batches:
        self._bucket_input_queue.put(b)
  ...
  def _TextGenerator(self, example_gen):
    ...
    while True:
      ...
      try:
          ...
          filename_text = self._GetExFeatureText(e,self._filename_key)
      except ValueError:
          tf.logging.error('Failed to get article or abstract from example')
          continue
      yield (article_text, abstract_text, filename_text)
